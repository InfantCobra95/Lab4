...
public class MovieController {
    ...
    public List<Movie> getMovies(@RequestParam(name="name", required=false, defaultValue="") String name, 
    ...
    public Movie updateMovie(@RequestBody Movie movie, @PathVariable("id") int id){
        for(int i = 0; i < movies.size(); i++){
            if (movies.get(i).getId() == id) {
                ...
                if (!movie.getImg().equals("")) {
                    movies.get(i).setImg(movie.getImg());
                }
    ...
    //No preguntes por que pongo 95, pondria Pipo en su lugar
